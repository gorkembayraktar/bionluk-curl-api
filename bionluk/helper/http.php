...
class Http{
    ...
    public function post(string $url,$data, $headers = [],$type = null){
        if($type == null){
          $type = HttpPostType::$QUERBUILDQUERY;
        }
        if($type == 'json'){
   ...
   public function put(string $url,$data, $headers = [],$type = null){
   ...
   public function mp3($url,$save_path){
    $ch = curl_init($url);
